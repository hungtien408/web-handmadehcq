...
public class ShoppingCart
{
        ...
        )
    {
        ...
        if (sessionCart == null)
        {
            ...
            dtCart.Columns.Add("SavePrice", typeof(double));
            dtCart.Columns.Add("OtherPrice");
            dtCart.Columns.Add("Quantity");

            dtCart.Rows.Add(new object[] { ProductID, ImageName, ProductName, ProductNameEn, ProductCode, ProductOrigin, Price, SavePrice, OtherPrice, Quantity });
        }
        else
        {
    ...
    public void UpdateQuantity(string ProductID, string Quantity,double Price)
    {
        if (HttpContext.Current.Session["Cart"] != null)
        {
            ...
            (from DataRow dr in dtCart.Rows
             where dr["ProductID"].ToString() == ProductID
             select dr).FirstOrDefault()["Price"] = Price;
